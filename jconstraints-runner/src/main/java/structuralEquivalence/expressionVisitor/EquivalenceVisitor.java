...
public class EquivalenceVisitor extends AbstractExpressionVisitor<Boolean, Expression> {
  ...
  public <E> Boolean visit(Constant<E> constant, Expression expression) {
    if (expression instanceof Constant) {
      ...
      return constant.getType().equals(expression.getType())
          && constantRight.getValue().equals(constant.getValue());
    }
    return false;
  }
  ...
  public Boolean visit(NumericBooleanExpression numericBooleanExpression, Expression expression) {
    if (expression instanceof NumericBooleanExpression) {
      ...
      return leftChecked
          && rightChecked
          && numericBooleanExpression.getComparator().equals(right.getComparator());
    }
    return false;
  }
  ...
  public Boolean visit(StringBooleanExpression stringBooleanExpression, Expression expression) {
    if (expression instanceof StringBooleanExpression) {
      ...
      return leftChecked
          && rightChecked
          && stringBooleanExpression.getOperator().equals(right.getOperator());
    }
    return false;
  }
  ...
  public Boolean visit(StringIntegerExpression stringIntegerExpression, Expression expression) {
    if (expression instanceof StringIntegerExpression) {
      ...
      boolean rightChecked =
          stringIntegerExpression.getRight() != null
              ? visit(stringIntegerExpression.getRight(), right.getRight())
              : right.getRight() == null;
      boolean offsetChecked =
          stringIntegerExpression.getOffset() != null
              ? visit(stringIntegerExpression.getOffset(), right.getOffset())
              : right.getOffset() == null;
  ...
  private boolean checkConcat(StringCompoundExpression left, StringCompoundExpression right) {
    ...
    for (int i = 0; i < left.getExpressions().length && expressionsChecked; i++) {
      expressionsChecked =
          expressionsChecked && visit(left.getExpressions()[i], right.getExpressions()[i]);
    }
  ...
  public Boolean visit(RegexCompoundExpression regexCompoundExpression, Expression expression) {
    if (expression instanceof RegexCompoundExpression) {
      ...
      return leftChecked
          && rightChecked
          && regexCompoundExpression.getOperator().equals(right.getOperator());
    }
    return false;
  }
  ...
  private Boolean checkOptional(
      RegexOperatorExpression regexOperatorExpression, RegexOperatorExpression right) {
  ...
  private Boolean checkRegexOperator(
      RegexOperatorExpression regexOperatorExpression, RegexOperatorExpression right) {
  ...
  private Boolean checkStrToRe(
      RegexOperatorExpression regexOperatorExpression, RegexOperatorExpression right) {
    boolean contentNull =
        regexOperatorExpression.getLeft() != null
                && right.getLeft() != null
  ...
  public <F, E> Boolean visit(CastExpression<F, E> castExpression, Expression expression) {
    if (expression instanceof CastExpression) {
      ...
      boolean fromType =
          castExpression.getCastOp().getFromClass().equals(right.getCastOp().getToClass());
      boolean toType =
          castExpression.getCastOp().getToClass().equals(right.getCastOp().getToClass());
  ...
  public Boolean visit(QuantifierExpression quantifierExpression, Expression expression) {
    ...
    boolean boundedChecked =
        quantifierExpression.getBoundVariables().equals(right.getBoundVariables());

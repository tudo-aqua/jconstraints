...
public class CVC5SolverContext extends SolverContext implements UNSATCoreSolver {
    ...
    private ConstraintSolver.Result secondTry(Valuation val) throws CVC5ApiException {
        //FIXME, this seems push pop realted in the CVC5 api. Otherwise, this should make no difference to using the context.
        Solver ctx2 = initSolver();
        HashMap<Variable, Term> vars2 = new HashMap<>();
        Term expr = new CVC5ExpressionGenerator(ctx2, vars2).generateExpression(ExpressionUtil.and(es.getCurrentExpression()));

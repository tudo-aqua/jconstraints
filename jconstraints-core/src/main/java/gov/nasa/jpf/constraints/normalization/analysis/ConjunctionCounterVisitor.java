...
public class ConjunctionCounterVisitor extends AbstractExpressionVisitor<Integer, Void> {
  ...
  private final int countConjunctions(Expression<?>... children) {
  ...
  public Integer visit(QuantifierExpression q, Void data) {
    return countConjunctions(q.getBody());
  }

  @Override
  protected <E> Integer defaultVisit(Expression<E> expression, Void data) {
    return countConjunctions(expression.getChildren());
  }

  @Override
  public Integer visit(LetExpression let, Void data) {
    return countConjunctions(let.flattenLetExpression());
  }

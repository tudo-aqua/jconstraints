...
public class ExpressionUtil {
  ...
  public static Expression<Boolean> simplifyAgressiv(Expression<Boolean> e) {

    Expression<Boolean> flat = e.accept(FlatExpressionVisitor.getInstance(), null);
    Expression restructured = flat.accept(SimplificationVisitor.getInstance(), null);
    Expression res = NumericSimplificationUtil.simplify(restructured);
  ...
  private static <E> void stripEntryPrefix(
      ValuationEntry<E> entry, String prefix, Valuation target) {
  ...
  public static Expression<Boolean> combine(
      LogicalOperator op,
  ...
  public static <T> Expression<T> restrict(
      Expression<T> expr, Predicate<? super Variable<?>> pred) {
    return ExpressionRestrictionVisitor.getInstance().apply(expr, pred);
  }

  public static <T> Expression<T> transformVars(
  ...
  public static String toParseableString(Expression<?> expression) {
    ...
    if (freeVars.isEmpty()) return expression.toString();
  ...
  private static <T> Expression<Boolean> addValuationEntryExpression(
      ValuationEntry<T> e, Expression<Boolean> expr) {
  ...
  @Deprecated
  public static Expression<Boolean> combine(
      Iterable<? extends Expression<Boolean>> expressions, LogicalOperator op) {

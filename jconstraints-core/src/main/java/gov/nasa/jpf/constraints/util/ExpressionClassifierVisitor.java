...
class ExpressionClassifierVisitor extends AbstractExpressionVisitor<ExpressionType, Void> {
  ...
  public <E> ExpressionType visit(NumericCompound<E> n, Void data) {
    ...
    switch (n.getOperator()) {
      case PLUS:
      case MINUS:
        if (lt == NONLINEAR || rt == NONLINEAR) return NONLINEAR;
        else if (lt == CONSTANT && rt == CONSTANT) return CONSTANT;
        else return LINEAR;
      case MUL:
      case DIV: // this could be made more succinct..
        if (lt == CONSTANT && rt == CONSTANT) return CONSTANT;
        else if (lt == NONLINEAR || rt == NONLINEAR) return NONLINEAR;
        else if (lt == LINEAR && rt == LINEAR) return NONLINEAR;
  ...
  @Override
  public <E> ExpressionType visit(
      gov.nasa.jpf.constraints.expressions.functions.FunctionExpression<E> f, Void data) {
    //  if(f.getType() instanceof BuiltinTypes.BoolType)
    //   return LINEAR;
    for (Expression<?> a : f.getArgs()) {
      if (visit(a) == NONLINEAR || visit(a) == LINEAR) return NONLINEAR;
    }
  ...
  public ExpressionType visit(PropositionalCompound n, Void data) {
    ...
    if (lt == NONLINEAR || rt == NONLINEAR) return NONLINEAR;
    if (lt == LINEAR || rt == LINEAR) return LINEAR;

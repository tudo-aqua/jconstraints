...
public class SinProperties implements FunctionProperties {
  ...
  public static final Function SIN_LOOKUP =
      new UnaryDoubleFunction("sin_lookup") {
        @Override
  ...
  public static final Function SIN_PI =
      new UnaryDoubleFunction("sin_pi") {
        @Override
        protected double doEvaluate(double value) {
          throw new UnsupportedOperationException("Not supported yet.");
        }
  ...
  private Expression<Boolean> sinPi() {
    ...
    return forall(eq(fexpr(SIN_PI, arg), ite(ifCond, thenExpr, elseExpr)), arg);
  }
  ...
  private Expression<Boolean> sin() {
    ...
    return forall(eq(fexpr(MathFunctions.SIN, arg), ite(ifCond, thenExpr, elseExpr)), arg);
  }

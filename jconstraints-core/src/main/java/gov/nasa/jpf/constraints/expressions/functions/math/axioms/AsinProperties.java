...
public class AsinProperties implements FunctionProperties {
  ...
  public static final Function ASIN_LOOKUP =
      new UnaryDoubleFunction("asin_lookup") {
        @Override
  ...
  private Expression<Boolean> asin() {
    ...
    return forall(eq(fexpr(MathFunctions.ASIN, arg), ite(ifCond, thenExpr, elseExpr)), arg);
  }
  ...
  public Expression<Boolean>[] getRangeBounds() {
    ...
    return array(
        bounds(MathFunctions.ASIN, constant(-0.5 * Math.PI), constant(0.5 * Math.PI), false));
  }
  ...
  public Expression<Boolean>[] getDefinition(Variable retValue, Expression... fargs) {
    ...
    return array(eq(lRet, lookupTable(lArg)), asin(fargs[0], retValue, lArg, lRet));
  }

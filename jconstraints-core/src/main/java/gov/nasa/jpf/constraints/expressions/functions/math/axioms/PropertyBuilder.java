...
public class PropertyBuilder {
  ...
  public static IfThenElse linearInterpolation(Expression x, double[] dVals, double[] fVals) {
    ...
    IfThenElse ite =
        ite(
            bounds(x, dVals[dVals.length - 3], dVals[dVals.length - 2], false, false),
            itp[itp.length - 2],
            itp[itp.length - 1]);
    ...
    for (int i = 3; i < dVals.length; i++) {
      ...
      ite =
          ite(
              bounds(x, dVals[dVals.length - i - 1], dVals[dVals.length - i], false, false),
              itp[itp.length - i],
              ite);
    }
    return ite;
  ...
  public static Expression<Boolean> bounds(
      Function func, Constant lower, Constant upper, boolean open) {
  ...
  public static Expression<Boolean> bounds(
      Expression x, double lower, double upper, boolean openLower, boolean openUpper) {
  ...
  public static <E> IfThenElse<E> ite(
      Expression<Boolean> ifCond, Expression<E> thenExpr, Expression<E> elseExpr) {

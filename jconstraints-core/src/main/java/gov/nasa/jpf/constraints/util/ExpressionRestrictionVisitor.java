...
class ExpressionRestrictionVisitor
    extends AbstractExpressionVisitor<Expression<?>, Predicate<? super Variable<?>>> {
  ...
  protected <E> Expression<?> defaultVisit(
      Expression<E> expression, Predicate<? super Variable<?>> data) {
    ...
    if (v1 == c1 && v2 == c2) return expression;

    if (v1 == null) {
      if (v2 != null && ExpressionUtil.containsVars(v2)) throw new MixedParamsException();
      return null;
    }
    if (v2 == null) {
      if (ExpressionUtil.containsVars(v1)) throw new MixedParamsException();
      return null;
    }

...
public class TypeContext {
  ...
  public TypeContext(boolean withBuiltin) {
    if (withBuiltin) addBuiltinTypes();
  }
  ...
  protected void registerType(Type<?> type) {
    ...
    if (otherNames != null) {
      for (String nam : otherNames) registerType(nam, type);
    }
    ...
    if (canonicalClass != null) {
      ...
      if (otherClasses != null) {
        for (Class<?> other : otherClasses) registerType(other, type);
      }
    }
  }
  ...
  public void addBuiltinTypes() {
    for (Type<?> t : BuiltinTypes.BUILTIN_TYPES) registerType(t);
  }

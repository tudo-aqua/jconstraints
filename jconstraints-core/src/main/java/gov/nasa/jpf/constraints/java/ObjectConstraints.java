...
public class ObjectConstraints {
  ...
  public static void addToValuation(String name, Object obj, Class<?> realClass, Valuation val) {
    if (realClass.isPrimitive()) {
      ...
      addInstanceToValuation(
          name, obj, Predicates.alwaysTrue(), val, new IdentityHashMap<Object, Boolean>());
    }
  }

  public static void addInstanceToValuation(
      String objName,
      Object obj,
  ...
  public static <T> Type<T> getPrimitiveType(Class<T> clazz) {
    if (clazz == Boolean.class) return (Type<T>) BuiltinTypes.BOOL;
    if (clazz == Byte.class) return (Type<T>) BuiltinTypes.SINT8;
    if (clazz == Character.class) return (Type<T>) BuiltinTypes.UINT16;
    if (clazz == Short.class) return (Type<T>) BuiltinTypes.SINT16;
    if (clazz == Integer.class) return (Type<T>) BuiltinTypes.SINT32;
    if (clazz == Long.class) return (Type<T>) BuiltinTypes.SINT64;
    if (clazz == Float.class) return (Type<T>) BuiltinTypes.FLOAT;
    if (clazz == Double.class) return (Type<T>) BuiltinTypes.DOUBLE;
    throw new IllegalArgumentException(
        "Class " + clazz.getName() + " is not a wrapper class for a primitive type");
  }

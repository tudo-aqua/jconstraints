...
public class ArrayBooleanExpression extends AbstractBoolExpression {
  ...
  public static ArrayBooleanExpression create(
      Expression<?> left, ArrayComparator operator, Expression<?> right) {
    return new ArrayBooleanExpression(left, operator, right);
  }

  public ArrayBooleanExpression(
      Expression<?> left, ArrayComparator comparator, Expression<?> right) {
    assert left != null
        && right != null
        && left.getType() instanceof ArrayType
        && right.getType() instanceof ArrayType;
    this.left = left;
